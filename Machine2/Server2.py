...
class FileServer:
    ...
    def getFileContent(self, filename):
        print("server2 getFileContent:", filename)
        try:
            with open(filename, 'r') as file:
                return file.read()
        except FileNotFoundError:
            print("server2 FileNotFoundError", filename)
    ...
    def forwardRequestToNextServer(self, filename):
        print("server2 forwardRequestToNextServer:", filename)
        if (self.next_server_address, self.next_server_port) in self.visited_servers:
            print("server2 Already visited server. Cannot forward request.")
            return None
        ...
        try:
            ...
            print("server2 Error forwarding request:", e)
            return None
...
def main():
    ...
    print(f"server2 started on {server_ip}:{server_port}. Press Ctrl+C to stop.")
